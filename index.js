...
// ── Темы (CSS-файлы из папки themes/) ─────────────────────────
const THEMES = [
    { id: 'default',    label: 'По умолчанию (без темы)', file: '' },
    { id: 'mono-gray',  label: 'Mono Gray (моно-серая + бургунди)', file: 'themes/theme-mono-gray.css' },
    { id: 'mono-glass', label: 'Mono Glass (стекло + бургунди)',    file: 'themes/theme-mono-glass.css' },
];

function extBaseUrl() {
    try {
        // ищем известный ресурс расширения, чтобы определить базовый путь
        const sels = [
            'link[href*="translator-suite"][href*=".css"]',
            'script[src*="translator-suite"][src*=".js"]',
        ];
    ...
    // запасной путь относительно SillyTavern
    return '/scripts/extensions/third-party/translator-suite/';
}

function applyTheme() {
    const id = (cfg().theme) || 'default';
    const theme = THEMES.find(t => t.id === id) || THEMES[0];
    const linkId = 'ts-theme-css';
    let link = document.getElementById(linkId);
    if (theme.file) {
        const href = extBaseUrl() + theme.file;
        if (!link) {
    ...
    // декоративная PNG-картинка справа в окнах с темой
    const img = (cfg().themeSideImg || '').trim();
    let varEl = document.getElementById('ts-theme-vars');
    if (img) {
        if (!varEl) { varEl = document.createElement('style'); varEl.id = 'ts-theme-vars'; document.head.appendChild(varEl); }
        varEl.textContent = `:root { --ts-side-img: url("${img.replace(/"/g, '\\"')}"); }`;
    } else if (varEl) {
        varEl.remove();
    }
}
...
if (_s.theme === undefined)            _s.theme              = 'default';
if (_s.themeSideImg === undefined)     _s.themeSideImg       = '';
...
<div id="ts-so">
  <div id="ts-sp">
    ...
    <div id="ts-stabs">
      ...
      <button class="ts-stab" data-t="theme"><i class="fa-solid fa-wand-magic-sparkles"></i> Тема</button>
    ...
    <!-- ── Тема ── -->
    <div class="ts-spane" id="ts-spane-theme" style="display:none">
      <div class="ts-sfield">
        <label><i class="fa-solid fa-wand-magic-sparkles"></i> Готовая тема оформления</label>
        <select id="ts-s-theme">${THEMES.map(t => `<option value="${t.id}">${esc(t.label)}</option>`).join('')}</select>
      </div>
      <div class="ts-sfield">
        <label><i class="fa-solid fa-image"></i> Декоративная PNG-картинка справа <span class="ts-optional">(опционально, для тем Mono Gray / Mono Glass)</span></label>
        <input type="text" id="ts-s-theme-img" placeholder='https://example.com/pic.png'>
      </div>
      <div class="ts-cssinfo">
        <i class="fa-solid fa-circle-info"></i>
        Темы подгружают свой CSS из папки <code>themes/</code> и переопределяют цвета,
        границы и фоны окон расширения. Палитру и Custom CSS можно использовать поверх темы.
      </div>
    </div>
...
function loadSettingsUI() {
    ...
    $('#ts-s-theme').val(c.theme || 'default');
    $('#ts-s-theme-img').val(c.themeSideImg || '');
...
function saveSettingsFromUI() {
    ...
    c.theme            = $('#ts-s-theme').val() || 'default';
    c.themeSideImg     = ($('#ts-s-theme-img').val() || '').trim();
...
function setupEvents() {
    ...
    // Тема — мгновенное применение
    $(document).on('change', '#ts-s-theme', function() {
        cfg().theme = $(this).val() || 'default';
        saveSettingsDebounced();
        applyTheme();
    });
    $(document).on('change input', '#ts-s-theme-img', function() {
        cfg().themeSideImg = ($(this).val() || '').trim();
        saveSettingsDebounced();
        applyTheme();
    });
